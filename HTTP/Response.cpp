...
int Response::handle_delete(int sock_fd) {
  ...
  if (stat(directoryFullPath.c_str(), &fileStat) == 0) {
    if (S_ISDIR(fileStat.st_mode)) {
      ...
      if (this->_request->getRequestedRessource()
              [this->_request->getRequestedRessource().size() - 1] != '/') {
...
std::vector<std::string> Response::chooseIndexes() {
  if (this->_request->getLocation() != NULL &&
      (this->_request->getLocation())->getIndex().size() != 0)
    ...
    if (S_ISDIR(fileStat.st_mode)) {
      ...
      if (this->_request->getRequestedRessource()
        ...
        throw(MovedPermanentlyException(
            this->_request->getRequestedRessource() + "/"));
      ...
      for (size_t i = 0; i < content.size(); i++) {
        ...
        if (stat(filePath.c_str(), &fileStat) == 0) // Check if stat succeeded
        {
          ...
          std::strftime(modified_str, sizeof(modified_str), "%Y-%m-%d %H:%M:%S",
                        modified_tm);
          indexFile +=
