...
void Request::appendBodyFile(const char *buffer, Request::size_type size) {
  if (!this->body_file_) {
    try {
      this->body_file_name_ =
          std::string(TMP_VAR_PATH) + helpers::timeBasedName(".request");
...
void Request::unchunckRequest(std::string &buffer) {
  ...
  if (this->chunk_received_ == 0) {
    ...
    size_to_write = (this->chunk_size_ < buffer.length() + chunk_received_
                         ? this->chunk_size_ - this->chunk_received_
                         : buffer.length());
...
void Request::setServer(Server *server) { this->server_ = server; }
...
const std::string &Request::getRequestedRessource() {
  return this->requestedRessource_;
}
...
std::string &Request::getRequestCgi() const {
  ...
  throw std::runtime_error("trying to get cgi but not set, check config file "
                           "or contact otossa for more details");
}
