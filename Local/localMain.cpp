      ...
      for (size_t i = 0; i < servers_data.size(); i++) {
        if (temp_data->getSockFd() == servers_data[i]->getSockFd()) {
          int client_sock = accept(
              servers_data[i]->getSockFd(),
              reinterpret_cast<sockaddr *>(&(servers_data[i]->getHostAddr())),
              reinterpret_cast<socklen_t *>(
                  &(servers_data[i]->getHostAddrlen())));
          if (client_sock == -1)
            std::cerr << "error: accept server " << std::endl;
          else {
            Client *u_data = new Client(client_sock, EVFILT_READ, 0);
            EV_SET(u_data->getChangePtr(), client_sock, EVFILT_READ,
                   EV_ADD | EV_ENABLE, 0, 0, u_data);
...
void serve() {
  ...
  for (size_t j = 0; j < servers.size(); j++) {
    ...
    fcntl(server_data->getSockFd(), F_SETFL,
          O_NONBLOCK); // if not the connection will block
    server_data->setHostAddrlen(sizeof(server_data->getHostAddr()));
    server_data->setHostAddr(std::stoi(servers[j]->getPort()));
    // .allowing the socket to be reusable..................................
    int opt = 1;
    if (setsockopt(server_data->getSockFd(), SOL_SOCKET, SO_REUSEADDR, &opt,
                   sizeof(opt)) < 0) {
      std::cerr << "error: setsockopt" << std::endl;
      continue;
    }
    opt = 1;
    if (setsockopt(server_data->getSockFd(), SOL_SOCKET, SO_NOSIGPIPE, &opt,
                   sizeof(opt)) < 0) {
      std::cerr << "error: setsockopt" << std::endl;
      continue;
    }
    // .binding the socket with an adress...................................
    if (bind(server_data->getSockFd(),
             reinterpret_cast<struct sockaddr *>(&(server_data->getHostAddr())),
             server_data->getHostAddrlen()) !=
        0) //? sockaddr is just an interface, is a sort of polymorphism
    {
    ...
    EV_SET(server_data->getChangePtr(), server_data->getSockFd(), EVFILT_READ,
           EV_ADD | EV_ENABLE, 0, 0, server_data);
