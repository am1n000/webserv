...
/*   Updated: 2023/03/31 00:26:34 by hchakoub         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Config::parse() {
  ...
  while(!tockenizer_->end()) {
    ...
    if(token == "server")
      ...
      // std::cout << "there is an include directive" << this->tockenizer_->getNoneEmptyLine() << std::endl;
  }
  // dev::br();
 ...
 */
void Config::test() {
  std::cout << this->mime_types_["mp4"] << std::endl;
}

void Config::cleenup() {
  // this is the method responsable for cleening the object i will let it empty for now 
}
...
Tockenizer* ConfigFile::tockenizer() {
   return this->tok_;
}
