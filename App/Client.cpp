...
Client::Client(int sockfd, int filtr, bool listen_sock) : _sockFd(sockfd), _filter(filtr), _isListeningSock(listen_sock), _postFileCreated(0)
{
	this->_changePtr = new struct kevent;
	this->req = new Request;
	this->resp = new Response;
...
void Client::reading(int kq)
{
	...
	if (this->req->content_lenght <= 0)
	{
		...
		EV_SET (this->_changePtr, this->_sockFd, EVFILT_WRITE, EV_ADD | EV_ENABLE, 0, 0, this);
		if (kevent(kq, this->_changePtr, 1, NULL, 0, NULL) == -1)
...
int	Client::getFilter()
{
	return (this->_filter);	
}

int&	Client::getHostAddrlen()
{
	return (this->_hostAddrlen);	
}

bool	Client::getIsListeningSock()
{
	return (this->_isListeningSock);	
}

bool	Client::getpostFileCreated()
{
	return (this->_postFileCreated);	
}

struct kevent*	Client::getChangePtr()
{
	return (this->_changePtr);	
}

struct sockaddr_in&	Client::getHostAddr()
{
